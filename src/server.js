...
async function runCollection(trigger = 'automatic') {
  ...
  try {
    ...
    memories.metadata.sources = results.map((r) => ({
      id: r.sourceId, name: r.source, records: r.records?.length || 0,
      error: r.error || null, collectedAt: r.collectedAt || new Date().toISOString()
    }));
    await writeMemories(memories);
    lastCollection = {
      ok: true, trigger, startedAt, finishedAt: new Date().toISOString(), total: parks.length,
      results: results.map((r) => ({ source: r.source, records: r.records?.length || 0, error: r.error || null }))
    };
    ...
    lastCollection = { ok: false, trigger, startedAt, finishedAt: new Date().toISOString(), error: error.message };
...
app.get('/api/v1/health', (req, res) => res.json({
  ok: true, service: 'Camping-car-API', time: new Date().toISOString(),
  collector: { running: collectionRunning, automatic: true, intervalMinutes, lastCollection, nextCollectionAt }
}));
...
app.get('/api/v1/parks/:id', auth, async (req, res) => {
  const d = await readMemories(); const p = d.parks.find((x) => x.id === req.params.id);
  if (!p) return res.status(404).json({ error: 'Aire introuvable' }); res.json(p);
});
app.get('/api/v1/search', auth, async (req, res) => {
  const q = String(req.query.q || '').toLowerCase().trim();
  if (!q) return res.status(400).json({ error: 'q est requis' });
  const d = await readMemories(); const parks = d.parks.filter((p) => JSON.stringify(p).toLowerCase().includes(q));
  res.json({ total: parks.length, parks });
});
app.get('/api/v1/nearby', auth, async (req, res) => {
  const lat = Number(req.query.lat), lon = Number(req.query.lon), radius = Number(req.query.radius || 25);
  ...
  const parks = d.parks.map((p) => ({ ...p, distanceKm: km(lat, lon, Number(p.location?.latitude), Number(p.location?.longitude)) })).filter((p) => p.distanceKm <= radius).sort((a, b) => a.distanceKm - b.distanceKm);
  res.json({ total: parks.length, radiusKm: radius, parks });
});
app.get('/api/v1/statistics', auth, async (req, res) => {
  const d = await readMemories(), bySource = {};
  for (const p of d.parks) { const s = p.source?.platform || 'unknown'; bySource[s] = (bySource[s] || 0) + 1; }
  res.json({ total: d.parks.length, bySource, generatedAt: d.metadata.generatedAt, lastCollection });
});

// Collecte publique : aucune API key n'est requise sur ces deux routes.
app.get('/api/v1/admin/collect/status', (req, res) => res.json({
  ok: true, running: collectionRunning, automatic: true, intervalMinutes, nextCollectionAt, lastCollection
}));
...
app.listen(port, host, async () => {
  ...
  console.log(`[COLLECT] Collecte automatique: toutes les ${intervalMinutes} minute(s).`);
